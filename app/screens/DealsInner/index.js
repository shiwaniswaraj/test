...
export   class DealsInner extends React.Component {
    ...
    componentDidMount(){
        ...
        this.setState({
          id,from,to,time,date,item,price,data:JSON.parse(data)
        });
        console.log(data);
    ...
    book=async()=>{
      ...
      if(!this.props.data){
          this.props.navigation.navigate("Login",{from:'DealsInner',...route.params})
      }
        var data={
        "action":"addbooking",
        ...
        "selleremail":this.props.aircarft_detail.sellerCompany.contactInfo.emails[0],
        "sellerphone":this.props.aircarft_detail.sellerCompany.contactInfo.phone,
        "selleraddress":this.props.aircarft_detail.sellerCompany.contactInfo.address,
        "sellerpostcode":this.props.aircarft_detail.sellerCompany.contactInfo.detailedAddress.postCode,
        "sellercity":this.props.aircarft_detail.sellerCompany.contactInfo.detailedAddress.city,
        "sellercountry":this.props.aircarft_detail.sellerCompany.contactInfo.detailedAddress.country.name,
        ...
        "yearofmake":this.props.aircarft_detail.lift.aircraftTailDetails.yearOfMake,
        "routes":[]
      }

      this.state.data.segments.map((e)=>{
            data.routes.push(
            {"startposition":e.startAirport.icao,"endposition":e.endAirport.icao,"amount":item.sellerprice.price,
            "currency":item.sellerprice.currency,"datetime":e.dateTime.date}
            )
      });
    ...
    render(){
      ...
      />
                ...
                <View style={[styles.whiteBottom,{borderBottomWidth:1,borderBottomColor:'#000'}]}>
                {
                data.segments.map((e)=>{
               return (
               ...
               return (
                <View style={styles.whiteBottomInner}>
                    ...
                    <Text>{e.dateTime.date} {e.dateTime.time}</Text>
                    </View>
               ...
               return (
                <View style={[styles.whiteBottomInner]}>
                    <View style={styles.botwrap}>
                    <View style={styles.lefbot}>
               <Text style={{color:'red',fontSize:19}}>{e.dateTime.time}</Text>
                            <Text style={{fontSize:10}}>{moment(e.dateTime.date).format('MMM Do YY')}</Text>
                    </View>
                    <Image source={iconPlane} style={styles.incoPLane}/>
                    <View style={styles.rightbot}>
                    <Text style={{color:'red',fontSize:19}}>{e.dateTime.time}</Text>
             ...
             
                {
                        data.segments.map((e)=>{
                   return (
                <View style={[styles.whiteBottom,{borderBottomWidth:1,borderBottomColor:'#000',flexDirection:'row',padding:0,paddingVertical:0}]}>
                <View style={[styles.whiteBottomInner,{flex:1,flexDirection:'column',paddingVertical:0,paddingBottom:0,borderRightColor:'#000',borderRightWidth:1}]}>
                ...
                <View style={[styles.whiteBottomInner,{flex:1,flexDirection:'column',paddingVertical:0,paddingBottom:0}]}>
                    <View style={{flexDirection:'row',padding:10}}>
                    <Image style={styles.pin} source={pin}/>
                    <Text >ARRIVAL</Text>
                    </View>
               ...
               return (
                ...
                <View style={[styles.whiteBottomInner,{flexDirection:'row'}]}>
                    <View style={{justifyContent:'center',alignItems:'center',flex:1}}>
                        <Text style={{textTransform:'uppercase'}}>seats selected</Text>
                    ...
                    <View style={{justifyContent:'center',alignItems:'center',flex:1}}>
                        <Text style={{textTransform:'uppercase'}}>FLIGHT TIME</Text>
                        <View style={{flexDirection:'row',marginTop:10}}>
                ...
                <View style={[styles.barBottom,{backgroundColor:'#000'}]}>
                    <Text style={{color:"#FFF",textAlign:"left"}}>AIRCRAFT DETAILS</Text>
                </View>
                <View style={[styles.whiteBottom]}>
                <View style={[styles.whiteBottomInner,{flexDirection:'column'}]}>
                    <View style={styles.det}><Text style={styles.dettl}>Model:</Text><Text  style={styles.dettr}>{title.lift.aircraftCategory}</Text></View>
                <View style={styles.det}><Text style={styles.dettl}>Class:</Text><Text  style={styles.dettr}>{title.lift.aircraftSuperType}</Text></View>
                    <View style={styles.det}><Text style={styles.dettl}>Max. Passengers:</Text><Text  style={styles.dettr}>{item.lift.maxPax}</Text></View>
                    <View style={styles.det}><Text style={styles.dettl}>Year of make:</Text><Text  style={styles.dettr}>{title.lift.aircraftTailDetails.yearOfMake}</Text></View>
                </View>
                <View style={{flex:1,justifyContent:'center',alignItems:'center'}}>  
                <Button onPress={()=>{this.book()}}   title="BOOK NOW" filled/>
                </View>
