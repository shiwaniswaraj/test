  ...
  return(<View style={styles.container}>
    <ScrollView style={{flex:1,padding:20}}>
        {msgData && msgData.map((e)=>{
          ...
          <Text style={{color:'#FFF',fontSize:19}}>{e.message}</Text>
          <Text style={{color:'#FFF',fontSize:14,marginTop:10}}>{moment(e.created_date).format('HH:mm a')}</Text>
        </View>)
        }else{
          return ( <View style={[styles.chat,styles.me]}>
            <Text style={{color:'#FFF',fontSize:19}}>{e.message}</Text>
          <Text style={{color:'#FFF',fontSize:14,marginTop:10,alignSelf:'flex-end'}}>{moment(e.created_date).format('HH:mm a')}</Text>
          </View>)
        }
      })}
    ...
    <View style={{padding:20}}>
      <ChatInput val={this.state.msg} onMessageChange={this.onMessageChange} sendMessage={this.sendMessage}/>
    </View>
  </View>
  )
}
