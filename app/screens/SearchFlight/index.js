...
import {KeyboardAvoidingView, StyleSheet, View,StatusBar,Image,Dimensions,ScrollView,FlatList,Platform,RefreshControl,ActivityIndicator,SafeAreaView } from "react-native";
...
import { TouchableHighlight, TouchableNativeFeedback, TouchableOpacity,TouchableWithoutFeedback } from "react-native-gesture-handler";
...
export  class SearchFlight extends React.Component {
    ...
    toggleOverlayCal=async (key)=>{
        ...
        const {showCal,selectedAirPort,departureData,arrivalData,time}=this.state;
        ...
        if(!selectedAirPort[key].departureData || 
    ...
    this.setState({
        ...
        await this.props.getDealPrice(departureData.icao,arrivalData.icao,moment(time).format("HH:mm"));
        this.setState({
            issearching:false,
            shows:true     
          })
...
searchData=()=>{
    // console.log()
    const {departureData,time,date,arrivalData,price,selectedAirPort}=this.state;
  ...
  var  retEnd=true;
    for(var i=0;i<selectedAirPort.length;i++){
        if(!selectedAirPort[i].departureData || 
    ...
    // console.log(data);
    this.props.navigation.navigate('Deals',{title:"Search Result",data:JSON.stringify(data)});
}
showTime=(key)=>{
...
clear=()=>{
    ...
    render(){
    const{visible,mode,departureData,arrivalData,date,time,pax}=this.state;


    

    const Item = ({ item }) => (
        <Button normal onPress={()=>{this.selectAirport(item)}} style={styles.item} >
        <View style={styles.itemLeft}>
        <Text style={styles.itmText}>{item.name},{item.city}</Text>
    ...
    const renderItem = ({ item }) => (
        <Item onStartShouldSetResponder={()=>{this.selectAirport(item)}} item={item} />
      );
    const {selectedAirPort,key}=this.state;
    ...
    return(<SafeAreaView style={[styles.container,{marginTop: StatusBar.currentHeight}]}>
     
      
        <View style={styles.mainBox}>
        
        <ScrollView contentContainerStyle={[this.state.selectedAirPort.length<=1 && {flex:1, justifyContent:'center'}]}>
            
       {
        ...
        <Button normal onPress={()=>{this.toggleOverlay('departure',e.key)}} style={[styles.fbox,e.departureData && styles.newbox]}>
            <Text style={[styles.colortxt,e.departureData && {color:'#000'}]}>{e.departureData?e.departureData.name:"Departure"}</Text>
            <Image source={Departure} style={styles.sideIcon}/>
            <Text style={[styles.neTxt,e.departureData && {color:'#000'}]}>{e.departureData?e.departureData.city:"TAP TO SELECT AIRPORT"}</Text>
           {e.departureData && <Text style={[{color:'#FFF'},e.departureData && {color:'#000'}]}>{e.departureData?e.departureData.icao:"TAP TO SELECT AIRPORT"}</Text>}
        </Button>
        <Button normal onPress={()=>{this.toggleOverlay('arrival',e.key)}} style={[styles.fbox,e.arrivalData && styles.newbox]}>
            <Text style={[styles.colortxt,e.arrivalData && {color:'#000'}]}>{e.arrivalData?e.arrivalData.name:"Arrival"}</Text>
            <Image source={Arrival} style={styles.sideIcon}/>
            <Text style={[styles.neTxt,e.arrivalData && {color:'#000'}]}>{e.arrivalData?e.arrivalData.city:"TAP TO SELECT AIRPORT"}</Text>
           {e.arrivalData && <Text style={[{color:'#FFF'},e.arrivalData && {color:'#000'}]}>{e.arrivalData?e.arrivalData.icao:"TAP TO SELECT AIRPORT"}</Text>}
        </Button>
      
        </View>

        <View style={styles.boxInner}>
        <View  style={[styles.fbox,styles.fboxIcon,e.date && {backgroundColor:'#FFF'}]} onStartShouldSetResponder={()=>{this.toggleOverlayCal(e.key)}}>
           <View style={styles.iconHolder} >
                <Octicons name="calendar" size={24} color={e.date?"black":"white"} />
    <Text style={[styles.iconText,e.date && {color:'#000'}]}>{e.date?moment(e.date).format("MMM Do YY"):"DATE"}</Text>
            </View>
        </View>
        <View  onStartShouldSetResponder={()=>{this.showTime(e.key)}} style={[styles.fbox,styles.fboxIcon,e.time && {backgroundColor:'#FFF'}]}>
           <View style={styles.iconHolder}>
                <Feather name="clock" size={24} color={e.time?"black":"white"} />
                <Text style={[styles.iconText,e.time && {color:'#000'}]}>{e.time?moment(e.time).format('h:mm a'):"TIME"}</Text>
            </View>
        </View>
        <View  onStartShouldSetResponder={()=>{this.showPax(e.key)}}  style={[styles.fbox,styles.fboxIcon,e.pax && {backgroundColor:'#FFF'}]}>
           <View style={styles.iconHolder}>
            <SimpleLineIcons name="printer" size={24} color={e.pax?"black":"white"} />
    <Text style={[styles.iconText,e.pax && {color:'#000'}]}>{e.pax?e.pax:"PAX"}</Text>
            </View>
        </View>
        </View>
        ...
        <View style={{paddingHorizontal:40,paddingVertical:20}}>
            ...
            thumbStyle={{ height: 30, width: 30, backgroundColor: '#FFF' }}        
            value={e.pax}
            maximumTrackTintColor="red"
            minimumTrackTintColor="#FFF"
            
            onValueChange={(val)=>{this.onChangePax(val)}}
            />
            <Text style={{alignSelf:'center',color:'#FFF'}}>PAX: {e.pax}</Text>
            </View>)
            }
        ...
        <View  style={[styles.fbox,styles.fboxIcon]}>
           <View style={[styles.icontwo,styles.iconHolder]}>
                <TouchableOpacity onPress={this.addReturn} style={{flex:1,alignItems:"center"}}>
                    <Text style={styles.iconText}>Return <FontAwesome style={styles.inconNew}  name="undo" size={14} color="white" /></Text>
                </TouchableOpacity>
                <View  onStartShouldSetResponder={()=>{this.addLeg()}} style={{flex:1,alignItems:"center"}}>
                <Text style={styles.iconText}>Add Leg <AntDesign style={styles.inconNew} name="plus"  size={14} color="white" /></Text>
                </View>
                <TouchableOpacity onPress={()=>{this.clear()}} style={{flex:1,alignItems:"center"}}>
                <Text style={styles.iconText}>Clear <AntDesign name="close"  style={styles.inconNew} size={14} color="white" /></Text>
                </TouchableOpacity>
            </View>
        </View>
        </View>
        ...
        <View style={[styles.botButton]}>
                ...
                <Button 
                color={
                    (mydiableStyle)? '#DDD':'red'
                }  
                onPress={()=>{this.searchData()}} title="SEARCH AIRCRAFT"/>
        </View>
        ...
        >
               <KeyboardAvoidingView
                ...
                style={styles.botnav}>
                    <TouchableOpacity onPress={this.toggleOverlay} style={{padding:10,alignSelf:'flex-end',zIndex:99999,paddingBottom:0,}}>
...
const mapStateToProps=state=>{
  ...
  const mapDispatchToProps = dispatch => {
    return{
        searchAirport:(filter,pgNo,perPage)=>dispatch(searchAirport(filter,pgNo,perPage)),
        getDealPrice:(startposition,endposition,time)=>dispatch(getDealPrice(startposition,endposition,time)),
