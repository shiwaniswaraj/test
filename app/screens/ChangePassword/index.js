...
export class ChangePassword extends React.Component {
	...
	changePassword = async () => {
		const { pass, newpass, cnfpass } = this.state;
		await this.props
			.changePassword(pass, newpass, cnfpass, this.props.data.id)
			.then((res) => {
				...
				<ScrollView containerStyle={{ flex: 1 }}>
					<View style={styles.listMain}>
						<Input
							...
							onChangeText={(val) => {
								this.handleTextChange("pass", val);
							}}
							black
						...
						<Input
							...
							onChangeText={(val) => {
								this.handleTextChange("newpass", val);
							}}
							secureTextEntry={true}
							black
						/>
						<Input
							...
							onChangeText={(val) => {
								this.handleTextChange("cnfpass", val);
							}}
							secureTextEntry={true}
							black
						/>
...
const mapDispatchToProps = (dispatch) => {
	return {
		changePassword: (oldpassword, newpassword, confirmpassword, id_custome) =>
			dispatch(
				passChange(oldpassword, newpassword, confirmpassword, id_custome)
			),
	};
};
