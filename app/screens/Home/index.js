...
import { StyleSheet, View,TouchableOpacity,Image,FlatList,Dimensions,ActivityIndicator } from "react-native";
...
import {getDealHome,getDealPrice,resetAircraft} from '../../redux/action/flight';
...
export   class Home extends React.Component {
  ...
  selectDate=(item,fromico,toico,time,from,to)=>{
    console.log("hey")
    this.setState({
      fromico,time,toico,from,to,item
    },()=>{
  ...
  searchData=async()=>{
    ...
    this.props.navigation.navigate('Deals',{title:"Search Result",to:from,from:to,time,date:moment(Date.now()).format('YYYY-MM-DD'),price:price,data:JSON.stringify(data)});
}


  toggleOverlayCal=async ()=>{
    const {showCal,fromico,time,toico}=this.state;
    ...
    await    this.props.getDealPrice(fromico,toico,time);
      this.setState({
        issearching:false,
        shows:true     
      }) 
...
dateSelect=(date,price)=>{
  ...
  const Item = ({ item }) => (
    <TouchableOpacity style={styles.flightBox} onPress={()=>{this.selectDate(item,item.fromicao,item.toicao,item.time,item.from,item.to)}}>
  ...
  return(<View style={styles.container}>
      ...
      <Image source={homeSlide} style={styles.image}/>
          ...
          <View style={styles.header}>
              <Image source={logo} style={{width:50,height:30}} resizeMode="contain"/>
          ...
          
   {
    ...
    <View style={{flexDirection:'row'}}>
        {this.props.homedeal && this.props.homedeal.slider.map((e,i)=>{
         return( <View style={[styles.ww,this.state.activeTab==i && styles.wwActive]}></View>)
        })}
      </View>
