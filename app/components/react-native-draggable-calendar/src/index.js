...
export class DraggableCalendar extends Component {
  ...
  _genDayData({startDate, endDate, availableStartDate, availableEndDate}) {
    ...
    const {price_cal}=this.props;
    // console.log(avilableDate)
    
    let result = {}, curDate = new Date(startDate);
    var c=[];
    var price=[];
    price_cal.map((e)=>{
      // if( moment(e.date).format("YYYY-DD-MM")==moment(curDate).format("YYYY-DD-MM")){
        c.push(moment(e.date).format("YY-DD-MM"));
        price.push(e.price);
      // }
    }) 
    ...
    while(curDate <= endDate) {
      ...
      // console.log(c.indexOf(moment(curDate).format("YY-DD-MM")))
      ...
      result[identifier].push({
        ...
        available:c.indexOf(moment(curDate).format("YY-DD-MM"))>-1,
        price:price[c.indexOf(moment(curDate).format("YY-DD-MM"))]
  ...
  _onPressDay=(date, available,price)=> {
    // console.log(date,price)
    if(date && available) {
      this.resetSelection([date, date]);
      this.props.toggleOverlayCal(date,price)
    }
  }
  ...
  _renderHeader() {
    ...
    return (
      <View style={[styles.headerContainer,{backgroundColor:'#646771',padding:10}, headerContainerStyle]}>
        {['SUN', 'MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT'].map(item => (
          <Text key={item} style={[styles.headerText,{color:'#FFF'}, headerTextStyle]}>{item}</Text>
        ))}
      </View>
    );
  }
  ...
  _renderDay(item, index) {
    const {renderDay,price_cal} = this.props;
    const styles = [
