...
export class Day extends PureComponent {
  ...
  _onPress() {
    ...
    onPress && onPress(data.date, data.available,data.price);
  }

  render() {
    const {data, renderDay,price_cal} = this.props;
              ...
              {data.date && (
                ...
                <Text style={[...usedDayTextStyle,{fontSize:11}, !data.available && {opacity: .6}]}  >
                  ...
                  {/* {this.props.price_cal.map((e)=>{
                    return moment(e.date).format("YYYY-DD-MM")==moment(data.date).format("YYYY-DD-MM")?'$'+e.price:'';
                  })} */}
                  </Text>
            </View>
